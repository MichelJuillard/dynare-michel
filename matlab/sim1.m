...
for iter = 1:options_.maxit
	...
	if ct_ == 1
		...
		oo_.y_simul(:,it_init:options_.periods+2) = oo_.y_simul(:,it_init:options_.periods+2)+options_.slowc*c ;
	else
		c = bksup1(ny,nrc) ;
		c = reshape(c,ny,options_.periods) ;
		oo_.y_simul(:,it_init:options_.periods+1) = oo_.y_simul(:,it_init:options_.periods+1)+options_.slowc*c ;
	end
